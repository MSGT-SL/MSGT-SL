...
def load_SL_data(cell_name, threshold=-3):
    if cell_name != "synlethdb":
        ...
        temp1 = set(np.unique(data['gene1']))
        temp2 = set(np.unique(data['gene2']))
...
def generate_torch_geo_data(cell_name, CCLE_feats_flag, CCLE_hidden_dim, node2vec_feats_flag, threshold, graph_input, attr, split_method, predict_novel_flag, training_percent):
    ...
    for graph_type in graph_input:
        if graph_type == "SL":
            temp_pos_graph_data = SL_data[SL_data['label'] == True]
            ...
            temp_all_genes = list(set(np.unique(graph_data['gene1'])) | set(np.unique(graph_data['gene2'])))
...
def evaluate_performance(label, pred):
    ...
    temp = pred>0.5
    temp1 = (pred>0.5)== label
    acc = ((pred>0.5)==label).sum() / len(pred)
    F1_score = metrics.f1_score(label, pred>0.5 , labels=[0, 1], average='macro')

    performance_dict = {"acc":acc.item(),"F1_score": F1_score,"AUC":AUC, "AUPR":AUPR, "precision@5":rank_score_5, "precision@10":rank_score_10}
