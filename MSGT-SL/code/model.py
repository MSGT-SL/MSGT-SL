...
class GCN_transformer_pool(torch.nn.Module):
    def __init__(self, in_channels, out_channels, num_graph, args):
        super(GCN_transformer_pool, self).__init__()
        self.num_graph = num_graph
        self.conv1 = GCNConv(in_channels, 2*out_channels)
        self.conv2 = GCNConv(2*out_channels, out_channels)
    ...
    def modified_transformer(self,transformer_input):
        ...
        transformer_input = torch.cat([transformer_input, torch.zeros((self.args.src_len - length, 64))], dim=0)
